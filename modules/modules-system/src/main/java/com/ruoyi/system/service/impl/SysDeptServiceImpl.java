...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements ISysDeptService
{
    ...
    {
        List<SysDept> beList = baseMapper.selectAllByTenantId(tenantId);
        //删除所有当前租户配置
        SysDept sysConfig = new SysDept();
        sysConfig.setTenantId(byTenantId);
        baseMapper.delete(new QueryWrapper<>(sysConfig));
        //插入所有配置
        beList.forEach(c->c.setTenantId(byTenantId));
        saveBatch(beList);
        List<SysDept> byList = baseMapper.selectAllByTenantId(byTenantId);
     ...
     */
    @Override
    @DataScope(userTable = false,deptTable = false,deptTableName = "d")
